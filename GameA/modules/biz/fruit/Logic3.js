...
define(function (require, exports, module) {
    ...
    function dealEscape(fruitObj,dt){
        ...
        if(isArrive){
            ...
            FruitPool.remove(fruitObj.entity.id);
    ...
    function dealHit(ammoObj,dt){
        ...
        FruitPool.remove(ammoObj.ammo.id);
        //调用击中函数
...
//        });
    ...
    function getTranUFOAndFruit(id,areaIndex){
        ...
        var groupObj=FruitPool.get(fruitObj.name,"FruitAndUFO","Group",createNode);   //获取一个Switch节点
//        groupNode=group.children[fruitObj.whichID];
//        groupNode.whichChoice=-1;       //隐藏水果飞船组
        groupNode=groupObj.node;
        tranUFONode=groupNode.choice[0];
        selNode=tranUFONode.children[0].children[0];
        fruitNode=groupNode.choice[1];
    ...
    function createNode(nodeName,nodePoolName,groupNodeName){
        var fruitObj=getFruitByFileName(nodeName);
        var str="";
        var fruitPath=path+"fruits/"+fruitObj.file;
        var ufoPath=path+"tranUFO/ufo/main.lyx";
        if(nodePoolName=="FruitAndUFO"){
            str="";
            str+=" Switch {whichChoice -1 choice [Transform {translation 0 -1.2 0 scale 0.5 0.5 0.5 children [Transform{translation 0 1.5 0 children[Switch{choice [ ";
            str+=" Material {diffuseColor .48 .53 .6 specularColor .32 .4 .4 emissiveColor .25 .27 .31 ambientIntensity .0667 shininess .54 transparency .55} ";
            str+=" Sphere {radius 2.2}]whichChoice -1}]} DEF tranUFO Inline {url \""+ufoPath+"\"}]} ";
            str+=" Transform {translation	0 0 0 children [Inline {url \""+fruitPath+"\"}]}]} ";
        }else if(nodePoolName=="FruitAmmo"){
            str="";
            str+=" Switch {whichChoice	-1 choice [Transform {translation	0 0 0 children [Inline {url \""+fruitPath+"\"}]}]} ";
        }
        return new SFNode(str).children[0];
    }
...
//        var fruitAmmoSwi=ammoGroup.children[assignFruit.whichID-1];
        var fruitAmmoObj=FruitPool.get(selectFruitName,"FruitAmmo","Group",createNode);
        var fruitAmmoSwi=fruitAmmoObj.node;
