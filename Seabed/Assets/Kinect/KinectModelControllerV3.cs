...
public class KinectModelControllerV3 : MonoBehaviour {
	...
	[HideInInspector]
	public Vector3[] boneRePos;
	[HideInInspector]
	public Quaternion[] boneReDir;
	...
	void Start () {
		...
		boneRePos = new Vector3[(int)Kinect.NuiSkeletonPositionIndex.Count];
	...
	void Update () {
		...
		if(sw.pollSkeleton()){
			...
			for(int ii=0; ii<20; ii++)
			{
				if( ((uint)Mask & (uint)(1 << ii) ) > 0 && (_nullMask & (uint)(1 << ii)) <= 0 )
				{
					boneRePos[ii] = _bones[ii].transform.position;
					boneReDir[ii] = _bones[ii].transform.rotation;
				}
			}
		}
	}
	...
	void RotateJoint(int bone) {
		...
		//pntSkeleton[0] = Kinect.NuiSkeletonData.SkeletonPositions[0];
